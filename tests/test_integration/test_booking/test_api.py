...
async def test_booking_add(room_id, date_from, date_to, db, authenticated_ac):
    ...
    if response.status_code == 200:
        ...
        assert len(await db.booking.get_in_params(room_id=room_id, date_from=from_date, date_to=to_date)) <= 5
    else:
        from_date = date.fromisoformat(date_from)
        to_date = date.fromisoformat(date_to)
        assert len(await db.booking.get_in_params(room_id=room_id, date_from=from_date, date_to=to_date)) == 5
...
async def delete_and_create_table_booking():
    ...
    async with engine_null_pool.begin() as conn:
        await conn.run_sync(
            lambda sync_conn: Base.metadata.drop_all(sync_conn, tables=[BookingsOrm.__table__], checkfirst=True))
        await conn.run_sync(
            lambda sync_conn: Base.metadata.create_all(sync_conn, tables=[BookingsOrm.__table__], checkfirst=True))

@pytest.mark.parametrize("room_id, date_from, date_to, count_bookings_for_user", [
...
async def test_add_and_get_bookings(room_id,
                                    date_from,
                                    date_to,
    ...
    await authenticated_ac.post("/booking",
                                           json={
                                               "room_id": room_id,
                                               "date_from": date_from,
                                               "date_to": date_to
